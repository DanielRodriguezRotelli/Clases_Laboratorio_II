using System;

/*
Crear un método estático que reciba una fecha y calcule el número de días 
que pasaron desde esa fecha hasta la fecha actual. Tener en cuenta los años bisiestos.

Pedir por consola la fecha de nacimiento de una persona (día, mes y año) y 
calcule el número de días vividos por esa persona hasta la fecha actual utilizando 
el método desarrollado anteriormente.

Ayudarse con las funcionalidades del tipo DateTime para resolver el ejercicio.
*/


namespace Clase02_Ejercicio08
{
    class Program
    {
        static void Main(string[] args)
        {
            Console.WriteLine("Ingrese su nombre: ");
            string nombre = Console.ReadLine();
            Console.WriteLine("Ingrese la fecha de su nacimiento: DD/mm/AA");
            string fecha = Console.ReadLine();
            string cantidadDias = CalculadorDeDias.CalcularDiasVivos(fecha);
            Console.WriteLine($"{nombre}, usted esta vivo hace {cantidadDias} dias");
            Console.Read();
        }
    }
}
